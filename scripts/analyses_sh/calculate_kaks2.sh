...
#SBATCH --mem=20GB
#SBATCH --job-name calculate_KaKs2
...
do
	...
	perl $SCRIPTS/calculate_Ka_Ks_pipeline/calculate_Ka_Ks_pipe2.pl \
		-d ../../ref/mcscanx/"$sample"-cds.fa \
		-g "$i" \
		-o kaks_results2/"$sample"."$pairs".kaks \
		-p $SCRIPTS/calculate_Ka_Ks_pipeline
	sed -i '2d' kaks_results2/"$sample"."$pairs".kaks
done
