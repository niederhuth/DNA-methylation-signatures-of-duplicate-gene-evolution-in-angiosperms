...
sub usage{
	die "Usage: ", basename($0)," -d <CDS.fasta> -g <Gene_pairs> -o <outfile_name> -p <path_to_scripts>\n";
}
...
&usage unless $db_file and  $gene_file and $out_file and $scripts_dir;
...
for(my $count = 0; $count < @contents; $count++){
	...
	$out->close;
	system "perl $scripts_dir/fa_prepareDATA4_kaks_calculator.pl $out_pre.fasta -p $scripts_dir > $out_pre.axt"
}
...
system "KaKs_Calculator -i $out_file.axt -m MA -o $out_file.KKC.format > $out_file.KKC.logfile";#modified Sep 25 2019
